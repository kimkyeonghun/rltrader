...
class Visualizer:
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, 
        ...
        x = np.arange(len(actions))  # 모든 차트가 공유할 x축 데이터, actions, num_stocks, outvals, exps, pvs의 크기가 모두 같음
        actions = np.array(actions)  # 에이전트의 행동 배열
        outvals = np.array(outvals)  # 정책 신경망의 출력 배열 -> matplotlib이 numpy 배열을 입력으로 받아서 변환
