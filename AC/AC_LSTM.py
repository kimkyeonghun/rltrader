...
class ACagent:
    ...
    def predict(self,sample):
        self.prob = self.actor.predict(np.array(sample).reshape((1, -1, self.input_dim)))[0]
        return self.prob
    ...
    def train_model(self,state,action,reward,next_state):
        value = self.critic.predict(np.array(state).reshape((1, -1, self.input_dim)))[0]
        next_value = self.critic.predict(np.array(next_state).reshape((1, -1, self.input_dim)))[0]
        ...
        state=np.array(state).reshape((1, -1, self.input_dim))
